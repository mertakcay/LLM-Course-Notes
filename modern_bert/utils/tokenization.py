...
class ModernBertTokenizer:
    def __init__(
        ...
        self.never_split.extend(self.additional_special_tokens)
    ...
    def encode(self, text: str, text_pair: Optional[str] = None, add_special_tokens: bool = True,
              padding: bool = False, truncation: bool = False) -> Dict:
        tokens = self.tokenize(text)
        if text_pair is not None:
            tokens_pair = self.tokenize(text_pair)
            result = self.prepare_for_model(
                tokens, 
                pair_tokens=tokens_pair, 
    ...
    def prepare_for_model(self, tokens: List[str], pair_tokens: Optional[List[str]] = None, 
                         add_special_tokens: bool = True, padding: bool = False, 
                         truncation: bool = False) -> Dict:
        if add_special_tokens:
            if pair_tokens is not None:
                result = self.create_token_type_ids_from_sequences(tokens, pair_tokens)
            else:
                result = self.build_inputs_with_special_tokens(tokens)
        else:
            result = {
        ...
        if truncation and len(result['input_ids']) > self.model_max_length:
            result['input_ids'] = result['input_ids'][:self.model_max_length]
            result['token_type_ids'] = result['token_type_ids'][:self.model_max_length]
        
        if padding:
            padding_length = self.model_max_length - len(result['input_ids'])
            if padding_length > 0:
                result['input_ids'] = result['input_ids'] + [self.pad_token_id] * padding_length
                result['token_type_ids'] = result['token_type_ids'] + [0] * padding_length
                result['attention_mask'] = [1] * len(result['input_ids']) + [0] * padding_length
            else:
...
class BasicTokenizer:
        ...
        for token in text.split():
            if token in self.never_split:
                ...
                if self.do_lower_case:
                    token = token.lower()
                    if self.strip_accents is not False:  # Modern feature: strip accents
    ...
    def _run_strip_accents(self, text: str) -> str:
        """Strips accents from a piece of text. Modern feature."""
        import unicodedata
